...
function displayTasks(item) {
    ...
    (item === null || item === void 0 ? void 0 : item.length) > 0
        ? item === null || item === void 0 ? void 0 : item.map((elem, index) => {
            if (taskDiv) {
                return (taskDiv.innerHTML += `<div class="flex py-1 h-fit mx-2 my-2 rounded-l-full rounded-r-full px-4 items-center  gap-4 text-white bg-gray-800 w-96 justify-between "><div><h1 class=${(elem === null || elem === void 0 ? void 0 : elem.isCompleted) ? "line-through" : "text-xl font-semibold"}>${elem.title} </h1></div>
