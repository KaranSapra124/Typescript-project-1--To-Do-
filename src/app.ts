...
function displayTasks(item: Task[]) {
  ...
  item?.map((elem: Task, index: number) => {
    if (taskDiv) {
      return (taskDiv.innerHTML += `<div class="flex py-1  my-2 rounded-l-full rounded-r-full px-4 items-center  gap-4 text-white bg-gray-800 w-96 justify-between "><div><h1 class=${
        elem?.isCompleted
          ? "line-through"
